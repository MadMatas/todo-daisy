...
export default function Tasks({tasks, setTasks}) {
    ...
    return (
        <>
            <input type="text" className="input block" placeholder="Search" value={searchQuery} onChange={(event) => {setSearchQuery(event.target.value)}} />
            <ul>
                {results.map((task) => (
                    <li key={task.id}>
                        ...
                        {task.task}
                        <button className="btn btn-error ml-3" onClick={() => {
                            remove(task);
                        }}>Remove</button>
                    </li>
                ))}
